  ...
  return (
    <div className="text-white  p-10 ">
      ...
      <div className="flex items-end mt-1">
        <h3 className="text-5xl text-[] font-semibold">the better</h3>
        {shortUrl && (
          <div className="bg-gray-300 p-1 px-4 ml-4 rounded-md flex">
            <a className="text-[#ed3f4a] text-xl" href={`/${shortUrl?.short}`}>
              {window.location.origin}/{shortUrl?.short}
            </a>
            <ClipboardDocumentIcon className="h-6 text-[#495e73]" />
          </div>
        )}
      </div>
    </div>
